...
interface StudentRow extends CreateStudentDto {
  rowError?: string;
}

const StudentImportPanel: React.FC = () => {
  ...
  const handleFileUpload = useCallback(async (file: File) => {
    try {
      ...
      const objects = csvRowsToObjects(csvRows, STUDENT_HEADERS);

      const parsed: StudentRow[] = objects.map((obj, idx) => {
        const errors: string[] = [];
  ...
  const handleImport = useCallback(async () => {
    ...
    setImporting(true);
    try {
      const res = await importApi.importStudents(validRows);
      setResult(res);
      if (res.failed === 0) {
        toast.success(`成功导入 ${res.success} 名学生`);
      } else {
